...
- 固定步长 120Hz 更新;渲染跟随浏览器 requestAnimationFrame,`ctx` 已应用相机变换后交给各 layer 绘制。
...
g.addParticles(x,y,{n,color,speed,life,size,grav,spread,dir,streak})
...
- 图标(被动,16×16):`p_might` `p_cd` `p_speed` `p_hp` `p_magnet` `p_xp` `p_gold` `p_armor`
...
export function spawnEnemy(g, typeId, x, y, o={hpMult,dmgMult,speedMult,elite,hpOverride}) -> e
...
// 敌人对象:{x,y,type,hp,maxHp,speed,dmg,r,flashT,hitT,kx,ky,elite,id}
// 命中玩家:p.takeDamage(e.dmg)(带 0.5s 每敌接触冷却)

// js/game/spawner.js
export function hpMultAt(t) -> number        // 普通怪血量曲线(600s×28,600s后继续增长)
export function eliteHpMultAt(t) -> number   // 精英独立血量曲线
export function spawnHpMultAt(typeId,t,o) -> number // 普通/精英/怪潮最终倍率
export function initSpawner(g)     // updater:按时间曲线成波刷怪(环带半径=视野外缘),难度递增
export function setEndless(g)      // 通关后无尽模式曲线

// js/game/boss.js
export const BOSS_DESIGNS = { boss_golem:{baseHp:24000}, boss_overlord:{baseHp:120000} }
export function bossHpAt(typeId, t) -> number // Boss 独立生命曲线,不复用普通怪曲线
export function initBoss(g)        // 300s 石像守卫(24000HP,半血狂怒);600s 无常尊者(120000HP,66%/33%三阶段)
// 最终 Boss 死亡 -> Bus.emit('runend',{victory:true});小Boss掉宝箱。Boss 血条写 g.boss

// js/game/upgrades.js
export const PASSIVES = { might:{name:'力量',icon:'p_might',maxLv:5,desc:lv描述}, ...9种 };
...
// data: {v,gold,chars:['knight'],settings:{sfx,music,shake,lowgfx,fpsShow},best:{time,kills,level,victory},totalRuns}
...
export const Screens = {
  ...
  buildMenu(cb:{onPlay,onToggle})                 // 主菜单含最佳纪录+设置开关
...
磁吸半径 60+;默认暴击 10%×1.6(游侠 20%×1.75);普通怪血量 0s×1→600s×28 且低基础血量有 735HP 保底(600s后继续增长);Boss 独立设计。

## 质量红线
- 手机优先:竖屏可玩、触控 ≥44px、120Hz 物理步进、同屏 200+ 敌人不卡
...
- 新增被动 `crit 暴击之眼`(icon: p_crit):暴击率 +6%/级,maxLv5(第9种被动,加入 rollChoices,只进升级池)
...
## 4. 后期压力软化(⚔️)
新生成普通怪血量按时间平滑成长：0s×1.00、60s×1.35、120s×1.90、300s×6.50、600s×28.00；低基础血量怪在后期至少 735HP。精英使用独立较缓曲线并叠加精英倍率；Boss 不复用此曲线。
